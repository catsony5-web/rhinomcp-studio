...
RHINO_REQUIRE_MATCH = os.getenv("RHINO_MCP_REQUIRE_MATCH", "0") == "1"
...
COMMANDS_SINCE: Dict[str, str] = {
    ...
    "create_wall": "0.5.0",
    "create_floor_slab": "0.5.0",
    "get_management_status": "0.6.0",
}
PLUGIN_UPDATE_ADVICE = (
    "Install the matching RhinoMCP Studio distribution, then restart Rhino. "
    "Run StudioMCPStart if the bridge did not start automatically."
)
SERVER_UPDATE_ADVICE = (
    "Install the matching RhinoMCP Studio distribution and reconnect the "
    "rhino-studio MCP server in Codex."
)
...
class ManagementPolicyError(RuntimeError):
    """The management gate refused a command before Rhino execution."""
...
class RhinoCommandOutcomeUnknownError(RuntimeError):
    """A mutation was sent but no reliable execution result was received."""


def uncertain_outcome(command_type: str, reason: str):
    return RhinoCommandOutcomeUnknownError(
        f"Outcome unknown for '{command_type}': {reason}. Rhino may have already "
        "changed the document. Do not repeat this mutation automatically. "
        "Reconnect and inspect get_document_summary/get_objects before deciding "
        "whether any further operation is needed."
    )
...
def rhino_startup_error_message(
    ...
    return (
        ...
        "Please start Rhino, run the Rhino command `StudioMCPStart`, then retry the MCP request."
    )
...
class RhinoConnection:
    ...
    def send_command(
        ...
        """
        params = params or {}
        # NaN/Infinity are not JSON and must never reach Rhino's geometry kernel.
    ...
    def _check_plugin_compatibility(self, command_type: str, params: Dict[str, Any]):
        ...
        if RHINO_REQUIRE_MATCH and command_type not in READONLY_RETRY_COMMANDS:
            reported = capabilities.get("version") if capabilities else None
            if (parse_version(reported) is None or server_version() is None
                    or parse_version(reported) != parse_version(server_version())):
                raise UnsupportedCommandError(
                    "Modeling was not sent: RhinoMCP Studio requires matching "
                    f"plugin and server versions (plugin={reported}, "
                    f"server={server_version()}). {PLUGIN_UPDATE_ADVICE}"
                )
    ...
    def _send_command_once(
        ...
        try:
            ...
            if response.get("status") == "error":
                ...
                if response.get("error_code") == "management_denied":
                    raise ManagementPolicyError(message)
            ...
            self.disconnect()
            if command_type not in READONLY_RETRY_COMMANDS:
                raise uncertain_outcome(command_type, "Timeout waiting for Rhino response")
            raise Exception(
            ...
            self.disconnect()
            if command_type not in READONLY_RETRY_COMMANDS:
                raise uncertain_outcome(command_type, "connection to Rhino was interrupted") from e
            raise TransientRhinoConnectionError(
                ...
                "and run the Rhino command `StudioMCPStart`."
            ) from e
            ...
            self.disconnect()
            if command_type not in READONLY_RETRY_COMMANDS:
                raise uncertain_outcome(command_type, "socket connection failed") from e
            raise TransientRhinoConnectionError(
                f"Connection to Rhino was interrupted at {self.host}:{self.port}. "
                "Retry the request. If this keeps happening, confirm Rhino is open "
                "and run the Rhino command `StudioMCPStart`."
            ...
            self.disconnect()
            if command_type not in READONLY_RETRY_COMMANDS:
                raise uncertain_outcome(command_type, "invalid JSON response from Rhino") from e
            ...
            raise
        except ManagementPolicyError:
            # This is a known refusal, not a transport failure or uncertain edit.
            # Keep local status diagnostics available and never retry around policy.
            raise
        except Exception as e:
...
def _reporting_errors(fn):
    ...
    path (start Rhino and run `StudioMCPStart`, fix a parameter, retry after a dropped
