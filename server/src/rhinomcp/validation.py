...
def _get_contracts_dir() -> Path:
    ...
    for contracts in (Path(__file__).parent / "contracts", CONTRACTS_DIR):
        if (contracts / "protocol.json").is_file():
            return contracts
    raise FileNotFoundError("Protocol contracts are missing. Reinstall RhinoMCP Studio.")
...
def validate_command(command_type: str, params: Dict[str, Any], raise_on_error: bool = True) -> bool:
    ...
    if not HAS_JSONSCHEMA:
        if raise_on_error:
            raise RuntimeError("jsonschema is missing. Reinstall RhinoMCP Studio.")
        return False
    ...
    try:
        ...
        if raise_on_error:
            raise
        logger.error(f"No schema found for {command_type}")
        return False
...
def validate_response(command_type: str, response: Dict[str, Any], raise_on_error: bool = True) -> bool:
    ...
    response_schema_map = {
        ...
        "get_management_status": "management_status.json",
