...
namespace RhinoMCPPlugin
{
    public class RhinoMCPServer
    {
                            ...
                            if (protocol == ClientProtocol.Framed)
                            {
                                ...
                                while (TryExtractFrame(pending, out string framedJson))
                                {
                                    ...
                                    catch (JsonException ex)
                                    {
                                        ...
                                        // This client thread is the only writer;
                                        // prior commands finish before reading the next frame.
                                        try
                                        {
                                            WriteMessage(stream, new JObject
                                            {
                                                ["status"] = "error",
                                                ["message"] = $"Invalid JSON in framed message: {ex.Message}"
        ...
        {
            string commandType = command["type"]?.Type == JTokenType.String
                ? command["type"].Value<string>() : null;
            if (string.IsNullOrWhiteSpace(commandType))
            {
                WriteMessage(stream, new JObject { ["status"] = "error", ["message"] = "A command type is required." }.ToString(), framed);
                return;
            }

            // Authorize on this background client thread, never Rhino's UI thread.
            // The TCP entry point is gated as well as calls coming from Python/MCP.
            bool diagnostic = ManagementClient.IsLocalDiagnostic(commandType);
            AuthorizationResult authorization = null;
            if (!diagnostic)
            {
                authorization = ManagementRuntime.Client.AuthorizeAsync(commandType).GetAwaiter().GetResult();
                if (!authorization.Allowed)
                {
                    WriteMessage(stream, ManagementDenied(authorization.Reason, authorization.Code).ToString(), framed);
                    return;
                }
            }

            // Only the client thread writes to its stream, and it waits for each UI
            // operation to complete. Pipelined requests cannot interleave responses.
            ...
            RhinoApp.InvokeOnUiThread(new Action(() =>
            {
                try
                {
                    if (!IsRunning())
                    {
                        completion.SetResult(new JObject { ["status"] = "error", ["message"] = "MCP server stopped before dispatch." });
                        return;
                    }
                    if (!diagnostic && !ManagementRuntime.Client.IsGrantCurrent(authorization.Grant, out string reason))
                    {
                        completion.SetResult(ManagementDenied(reason, "authorization_expired"));
                        return;
                    }
                    completion.SetResult(ExecuteCommand(command));
                }
                catch (Exception e)
                {
                    completion.SetResult(new JObject { ["status"] = "error", ["message"] = e.Message });
                }
            }));
            JObject response = completion.Task.GetAwaiter().GetResult();
            WriteMessage(stream, JsonConvert.SerializeObject(response), framed);
        }

        private static JObject ManagementDenied(string reason, string code) => new JObject
        {
            ["status"] = "error",
            ["error_code"] = "management_denied",
