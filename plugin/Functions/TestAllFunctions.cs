...
public partial class RhinoMCPFunctions
{
    ...
    /// <summary>
    /// Focused regression checks. Call only in a separate empty test document.
    /// No document Undo or name-based cleanup is used, and these tests are not
    /// registered as modeling tools.
    /// </summary>
    public JObject TestModelingSafety()
    {
        var doc = RhinoDoc.ActiveDoc;
        var activeObjects = new ObjectEnumeratorSettings
        {
        ...
        // ObjectTable.Count includes undo-held deleted objects, and LINQ Any()
        // can use that count. Enumerate the explicit live-object filter instead.
        if (doc == null || doc.Objects.GetObjectList(activeObjects).Any())
            throw new InvalidOperationException("TestModelingSafety requires a separate empty Rhino document");

        var results = new JObject();
        var ownedIds = new HashSet<Guid>();
        var testPrefix = "MCPSafety_" + Guid.NewGuid().ToString("N");
        var tolerance = Math.Max(doc.ModelAbsoluteTolerance, 1e-6);

        Guid Track(Guid id)
        {
            if (id == Guid.Empty) throw new Exception("Could not create test fixture");
            ownedIds.Add(id);
            return id;
        }

        Guid BoxAt(double x, string suffix)
        {
            var box = new Box(new BoundingBox(new Point3d(x, 0, 0), new Point3d(x + 10, 10, 10)));
            return Track(doc.Objects.AddBox(box, new ObjectAttributes { Name = testPrefix + suffix }));
        }
        ...
        void ExpectFailure(Action action)
        {
            ...
            throw new Exception("Expected operation to fail without changing the model");
        }
        ...
        void Run(string name, Action action)
        {
            try
            {
                Require(!doc.Objects.GetObjectList(activeObjects).Any(), "Previous test left objects; refusing to modify the document");
                action();
                results[name] = new JObject { ["status"] = "pass" };
            }
            catch (Exception error)
            {
                results[name] = new JObject { ["status"] = "fail", ["error"] = error.Message };
            }
            finally
            {
                foreach (var id in ownedIds)
                {
                    ...
                    if (obj != null && !obj.IsDeleted && !doc.Objects.Delete(obj, true, true))
                        results[name] = new JObject { ["status"] = "fail", ["error"] = $"Could not remove test fixture {id}" };
                }
                ownedIds.Clear();
            }
        }

        foreach (var operation in new[] { "union", "difference", "intersection" })
        {
            Run("boolean_" + operation + "_volume_and_sources", () =>
            {
                var a = BoxAt(0, "A");
                var b = BoxAt(5, "B");
                JObject answer;
                if (operation == "difference")
                    answer = BooleanDifference(new JObject { ["base_id"] = a.ToString(), ["subtract_ids"] = new JArray(b.ToString()), ["name"] = testPrefix + "Result" });
                else
                {
                    var parameters = new JObject { ["object_ids"] = new JArray(a.ToString(), b.ToString()), ["name"] = testPrefix + "Result" };
                    answer = operation == "union" ? BooleanUnion(parameters) : BooleanIntersection(parameters);
                }
                var ids = (JArray)answer["result_ids"];
                foreach (var id in ids) Track(Guid.Parse(id.ToString()));
                Require(ids.Count == 1, "Expected one Boolean result");
                var brep = doc.Objects.FindId(Guid.Parse(ids[0].ToString()))?.Geometry as Brep;
                Require(brep != null && brep.IsValid && brep.IsSolid, "Boolean result must be a valid solid");
                using var volume = VolumeMassProperties.Compute(brep);
                var expectedVolume = operation == "union" ? 1500.0 : 500.0;
                Require(volume != null && Math.Abs(volume.Volume - expectedVolume) <= tolerance, "Boolean volume differs from expected overlap");
                Require(!SnapshotObjectIds(doc).Contains(a) && !SnapshotObjectIds(doc).Contains(b), "Successful Boolean retained a source unexpectedly");
            });
        }

        Run("boolean_locked_source_preserves_all_inputs", () =>
        {
            var a = BoxAt(0, "A");
            var b = BoxAt(5, "B");
            Require(doc.Objects.Lock(b, true), "Could not lock test input");
            var before = SnapshotObjectIds(doc);
            ExpectFailure(() => BooleanUnion(new JObject { ["object_ids"] = new JArray(a.ToString(), b.ToString()) }));
            Require(before.SetEquals(SnapshotObjectIds(doc)), "Rejected Boolean changed the object set");
            Require(doc.Objects.FindId(b).IsLocked, "Rejected Boolean changed source locking");
        });

        Run("boolean_partial_delete_restores_exact_sources", () =>
        {
            var a = BoxAt(0, "A");
            var b = BoxAt(5, "B");
            var before = SnapshotObjectIds(doc);
            var sourceSerial = doc.Objects.FindId(a).RuntimeSerialNumber;
            var lockTriggered = false;
            EventHandler<RhinoObjectEventArgs> lockNextSource = (_, args) =>
            {
                if (args.ObjectId == a)
                    lockTriggered = doc.Objects.Lock(b, true);
            };
            RhinoDoc.DeleteRhinoObject += lockNextSource;
            try
            {
                ExpectFailure(() => BooleanUnion(new JObject { ["object_ids"] = new JArray(a.ToString(), b.ToString()), ["name"] = testPrefix + "Result" }));
            }
            finally
            {
                RhinoDoc.DeleteRhinoObject -= lockNextSource;
            }
            Require(lockTriggered, "Deletion failure fixture was not exercised");
            Require(before.SetEquals(SnapshotObjectIds(doc)), "Rollback did not restore the original object set");
            var restored = doc.Objects.FindId(a);
            Require(restored.RuntimeSerialNumber == sourceSerial && restored.Attributes.Name == testPrefix + "A", "Rollback recreated or changed the original object");
            using var restoredBrep = restored.Geometry is Extrusion extrusion
                ? extrusion.ToBrep() : (restored.Geometry as Brep)?.DuplicateBrep();
            Require(restoredBrep != null && restoredBrep.IsSolid, "Restored source must be a solid");
            using var volume = VolumeMassProperties.Compute(restoredBrep);
            Require(volume != null && Math.Abs(volume.Volume - 1000) <= tolerance, "Restored source geometry changed");
        });

        Run("invalid_result_rejected_before_document_changes", () =>
        {
            var a = BoxAt(0, "A");
            var before = SnapshotObjectIds(doc);
            using var valid = new Box(new BoundingBox(Point3d.Origin, new Point3d(1, 1, 1))).ToBrep();
            using var invalid = new Brep();
            ExpectFailure(() => CommitBrepResults(doc, new[] { valid, invalid }, testPrefix + "Result", "Regression test", new[] { doc.Objects.FindId(a) }));
            Require(before.SetEquals(SnapshotObjectIds(doc)), "Invalid result caused a partial commit");
        });

        Run("extrude_cap_contract", () =>
        {
            using var profile = new Rectangle3d(Plane.WorldXY, 10, 10).ToNurbsCurve();
            var profileId = Track(doc.Objects.AddCurve(profile));
            var result = ExtrudeCurve(new JObject { ["curve_id"] = profileId.ToString(), ["direction"] = new JArray(0, 0, 10), ["cap"] = true });
            var resultId = Track(Guid.Parse(result["result_id"].ToString()));
            var solid = doc.Objects.FindId(resultId)?.Geometry as Brep;
            Require(solid != null && solid.IsValid && solid.IsSolid, "Capped extrusion is not a valid solid");
            using var volume = VolumeMassProperties.Compute(solid);
            Require(volume != null && Math.Abs(volume.Volume - 1000) <= tolerance, "Extrusion volume is incorrect");
            var lineId = Track(doc.Objects.AddLine(Point3d.Origin, new Point3d(10, 0, 0)));
            var before = SnapshotObjectIds(doc);
            ExpectFailure(() => ExtrudeCurve(new JObject { ["curve_id"] = lineId.ToString(), ["direction"] = new JArray(0, 0, 10), ["cap"] = true }));
            ExpectFailure(() => ExtrudeCurve(new JObject { ["curve_id"] = profileId.ToString(), ["direction"] = new JArray(0, 0, 0), ["cap"] = false }));
            Require(before.SetEquals(SnapshotObjectIds(doc)), "Rejected extrusion left unexpected geometry");
            var openResult = ExtrudeCurve(new JObject { ["curve_id"] = lineId.ToString(), ["direction"] = new JArray(0, 0, 10), ["cap"] = false });
            var openId = Track(Guid.Parse(openResult["result_id"].ToString()));
            Require(doc.Objects.FindId(openId).Geometry.IsValid, "Explicit open extrusion failed");
        });

        Run("modify_locked_target_reports_failure_without_changes", () =>
        {
            var a = Track(doc.Objects.AddPoint(Point3d.Origin, new ObjectAttributes { Name = testPrefix + "Locked" }));
            var b = Track(doc.Objects.AddPoint(new Point3d(20, 0, 0)));
            Require(doc.Objects.Lock(a, true), "Could not lock modification fixture");
            var request = new JObject { ["id"] = a.ToString(), ["new_name"] = "ShouldNotBeApplied", ["translation"] = new JArray(10, 0, 0) };
            ExpectFailure(() => ModifyObject(request));
            Require(doc.Objects.FindId(a).Attributes.Name == testPrefix + "Locked", "Rejected modification changed attributes");
            Require(((Rhino.Geometry.Point)doc.Objects.FindId(a).Geometry).Location.DistanceTo(Point3d.Origin) <= tolerance, "Rejected modification moved a locked object");
            var batch = ModifyObjects(new JObject
            {
                ["objects"] = new JArray(request.DeepClone(), new JObject { ["id"] = b.ToString(), ["translation"] = new JArray(10, 0, 0) })
            });
            Require((int)batch["success_count"] == 1 && (int)batch["failure_count"] == 1, "Batch must report the locked target failure");
            Require(Math.Abs(((Rhino.Geometry.Point)doc.Objects.FindId(b).Geometry).Location.X - 30) <= tolerance, "Valid batch target was not modified");
        });

        Run("modify_all_once_without_mutating_request", () =>
        {
            var deletedId = Track(doc.Objects.AddPoint(new Point3d(-20, 0, 0)));
            var deleted = doc.Objects.FindId(deletedId);
            Require(doc.Objects.Delete(deleted, true, false) && deleted.IsDeleted, "Could not retain a deleted test object in undo history");
            var a = Track(doc.Objects.AddPoint(Point3d.Origin));
            var b = Track(doc.Objects.AddPoint(new Point3d(20, 0, 0)));
            var parameters = new JObject
            {
                ["all"] = true,
                ["objects"] = new JArray(new JObject { ["id"] = a.ToString(), ["translation"] = new JArray(10, 0, 0) })
            };
            var originalRequest = parameters.DeepClone();
            var result = ModifyObjects(parameters);
            Require((int)result["success_count"] == 2 && (int)result["failure_count"] == 0, "Each target must be modified exactly once");
            Require(deleted.IsDeleted, "Batch modification restored a deleted object");
            Require(JToken.DeepEquals(parameters, originalRequest), "Batch modification mutated the input request");
            Require(Math.Abs(((Rhino.Geometry.Point)doc.Objects.FindId(a).Geometry).Location.X - 10) <= tolerance, "Template target was transformed more than once");
            Require(Math.Abs(((Rhino.Geometry.Point)doc.Objects.FindId(b).Geometry).Location.X - 30) <= tolerance, "Second target transform is incorrect");
            parameters["all"] = false;
            ModifyObjects(parameters);
            Require(Math.Abs(((Rhino.Geometry.Point)doc.Objects.FindId(b).Geometry).Location.X - 30) <= tolerance, "all=false modified an unrelated object");
        });
