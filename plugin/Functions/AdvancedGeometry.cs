...
public partial class RhinoMCPFunctions
{
    ...
    public JObject Loft(JObject parameters)
    {
        ...
        var resultIds = CommitBrepResults(doc, breps, name, "Loft");
    ...
    public JObject ExtrudeCurve(JObject parameters)
    {
        ...
        if (!directionVec.IsValid || directionVec.IsZero)
            throw new ArgumentException("Extrude requires a finite, non-zero direction vector");
        if (cap && (!curve.IsClosed || !curve.IsPlanar(doc.ModelAbsoluteTolerance)))
            throw new ArgumentException("Capped extrusion requires a closed planar curve; use cap=false for an open surface");

        // Create extrusion
        using var extrudedSurface = Surface.CreateExtrusion(curve, directionVec);
        if (extrudedSurface == null || !extrudedSurface.IsValid)
        ...
        if (cap)
        {
            using var brep = extrudedSurface.ToBrep();
            using var cappedBrep = brep?.CapPlanarHoles(doc.ModelAbsoluteTolerance);
            resultId = CommitBrepResults(doc, new[] { cappedBrep }, name,
                "Capped extrusion", requireSolid: true)[0].ToString();
        }
        else
        {
            var id = doc.Objects.AddSurface(extrudedSurface, attr);
            if (id == Guid.Empty)
                throw new InvalidOperationException("Extrusion could not be inserted into the document");
        ...
        try
        {
            foreach (var brep in breps)
            {
                if (brep == null || !brep.IsValid)
                    throw new InvalidOperationException("Sweep produced invalid geometry; no results were added");
                if (!capPlanarEnds)
                {
    ...
    public JObject Pipe(JObject parameters)
    {
        ...
        var resultIds = CommitBrepResults(doc, breps, name, "Pipe", requireSolid: cap);
